...
def calculateRawPoints(games):
    ...
    print("Games in Consideration: ", games)
    for game in games:
        # print(game)
        givePoints(game)
...
def initializeAllPointsByTeamDict(team):
        ...
        if not team in allPointsByTeamDict:
            allPointsByTeamDict[team] = { "RawPoints": 0, "H2H" : 0, 'PDPoints': 0, 'TotalPoints': 0, 'PD': 0, 'GoalsInFavor': 0, 'PointsScoredOn': 0, 'Wins': 0 , 'Losses': 0, 'Ties': 0, 'RankNumber': 0, 'H2H2points': 0, 'VBPD': 0}
        return 0
...
def duplicateChecker(allPointsByTeamDict, keyName, tournament_id = None , poolName = None, cur = None):
    ...
    for team, allPointsDict in allPointsByTeamDict.items():
        ...
        if keyName == "":
            ...
            totalPointsByTeamDict = allPointsDict['RawPoints'] + (allPointsDict['H2H'] * 0.1) + (allPointsDict['PD'] * 0.0001) + (allPointsDict['H2H2points'] * 0.00001) + (allPointsDict['GoalsInFavor'] * 0.00000001) + (allPointsDict['VBPD'] * 0.00000000001)
...
def rankBasedOn(mydb, cur, games, stillTie, tournament_id, poolName):
    ...
    for qual,team in enumerate(allPointsByTeamDict):
        ...
        totalPointsByTeamDict[team] = allPointsByTeamDict[team]['RawPoints'] + (allPointsByTeamDict[team]['H2H'] * 0.1) + (allPointsByTeamDict[team]['PD'] * 0.0001) + (allPointsByTeamDict[team]['H2H2points'] * 0.00001) + (allPointsByTeamDict[team]['GoalsInFavor'] * 0.00000001) + (allPointsByTeamDict[team]['VBPD'] * 0.00000000001)
